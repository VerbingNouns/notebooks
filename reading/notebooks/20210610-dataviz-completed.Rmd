...
ggplot(penguins, 
       aes(x = bill_length_mm,
           fill = species)) +
  theme_bw() +
  geom_histogram(binwidth = 1, position = "dodge") + # this creates a histogram with two bins per mm
  #geom_density(alpha = .5, position = "dodge") + # stack or dodge
  NULL
```
...
# simdat <- read.csv(header = TRUE, file.choose())
...
simdat %>% 
  ...
  theme_bw() +
  #geom_point() + # what geometry should you use for a scatter plot?
  geom_smooth(aes(colour = freq, linetype = gram),
              method = "lm") +# can you implement geom_smooth() here?
  NULL
```
...
+
  geom_boxplot() +# what geometry should you use for a box plot?
  # can you implement facet_grid() or facet_wrap() here?
  theme_bw() +
  facet_grid(rating~region) +
  NULL
```
...
```{r, message=FALSE}
simdat %>% 
  filter(region == 1) %>% # ensure you are only looking at one of the five regions
  group_by(freq, gram, rating) %>% # group your data by frequency, grammaticality, and rating
  summarise(total = n()) %>% # summarise your data so you can get total counts of each grouping
  mutate(condition = paste(gram,freq)) %>% # create ("mutate") a column that combines the grammaticality and frequency condition names to make 4 unique names
  ggplot(aes(x = condition, y = total, fill = rating)) + # add aesthetics for x-axis, y-axis, and fill
  geom_bar(stat = "identity") + #implement the appropriate geometry for a bar plot, including arguments for 'position' and 'stat'
  NULL
```

```{r}
paste0("Hello", "I am called", "Lauren.")

interaction(c("A", "A", "B", "B"), c("C", "C", "D", "D"))

simdat %>% 
  mutate(interaction = interaction(freq,gram))
```
...
simdat %>% 
  ...
  ggplot(aes(x = region, 
             ...
             colour = gram, 
             linetype = freq)) +
  theme_bw() + # this one makes the background white
  geom_point() +
  geom_path() +
