...
date: "Last updated: 01 April 2019"
output:
  html_document:
    theme: sandstone
    toc: yes
...
As in typical LMERs that one might produce using `lme4`, we can see AIC=`r round(summary(lmer.rate.max)$AIC[[1]],2)`, BIC=`r round(summary(lmer.rate.max)$AIC[[1]],2)`, and Log Likelihood=`r round(summary(lmer.rate.max)$AIC[[3]],2)`. We're presented with various residuals and a summary of the random effects (which are important but I will skip for now).
...
In this graph below, we can see the probability curves for each of the seven ratings. In particular, we can now see that the *Baseline+Present* condition and the *Baseline+Absent* condition are not rated in precisely the same manner: *Baseline+Present* is more likely to receive a rating of 7 and less like to receive a rating of 5 than *Baseline+Absent*.
...
linetypes <- c("one" = "solid",
               "two" = "11",
               "three" = "12",
               "four" = "13",
               "five" = "14",
               "six" = "15",
               "seven" = "16") # establish the lty-rating level correspondence
