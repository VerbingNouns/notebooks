...
```
```{r, echo=FALSE}
head(data[data$experiment=="second",],10)
```
...
It starts to look messy and hard to read pretty quickly though. The [next tutorial](tutorial-2.html) will introduce you to a different way of doing this that is easier to read.
...
* `col` lets you change the colour of the line or point. There are [a large number of pre-named colours](http://research.stowers.org/mcm/efg/R/Color/Chart/).
...
I will discuss GitHub more in a later tutorial. It is very difficult to break into but very simple to use effectively once you know the basics.

---

<big>[Go to Tutorial 2 →](tutorial-2.html)</big>